...
class Delegate {
  ...
  _internal_unsubscribe(callback) {
    const index = this._private__listeners.findIndex(
      (listener) => callback === listener._internal_callback
    );
    if (index > -1) {
  ...
  _internal_unsubscribeAll(linkedObject) {
    this._private__listeners = this._private__listeners.filter(
      (listener) => listener._internal_linkedObject !== linkedObject
    );
  }
  _internal_fire(param1, param2, param3) {
    const listenersSnapshot = [...this._private__listeners];
    this._private__listeners = this._private__listeners.filter(
      (listener) => !listener._internal_singleshot
    );
    listenersSnapshot.forEach((listener) =>
      listener._internal_callback(param1, param2, param3)
    );
  }
  ...
  _internal_options() {
    ...
    if (
      ...
      rendererOptions._internal_paddingInner =
        (currentFontSize / 12) * rendererOptions._internal_tickLength;
    ...
    rendererOptions._internal_paneBackgroundColor =
...
class MediaCoordinatesPaneRenderer {
  _internal_draw(target, isHovered, hitTestData) {
    target.useMediaCoordinateSpace((scope) =>
      this._internal__drawImpl(scope, isHovered, hitTestData)
    );
  }
  _internal_drawBackground(target, isHovered, hitTestData) {
    target.useMediaCoordinateSpace((scope) =>
      this._internal__drawBackgroundImpl(scope, isHovered, hitTestData)
    );
  }
  ...
  _private__updateImpl() {
    ...
    serieses.forEach((s, index) => {
      ...
      data._internal_backColor =
        (_a = seriesData._internal_borderColor) !== null && _a !== void 0
          ? _a
          : this._private__chartModel._internal_backgroundColorAtYPercentFromTop(
              data._internal_items[0]._internal_y /
...
class PriceAxisViewRenderer {
  ...
  _internal_draw(target, rendererOptions, textWidthCache, align) {
    if (
      !this._private__data._internal_visible ||
    ...
    const geometry = target.useBitmapCoordinateSpace((scope) => {
      ...
      const geom = this._private__calculateGeometry(
        scope,
        rendererOptions,
        textWidthCache,
        align
      ...
      const drawLabelBody = (labelBackgroundColor, labelBorderColor) => {
        if (geom._internal_alignRight) {
          drawRoundRectWithInnerBorder(
            ctx,
            gb._internal_xOutside,
            gb._internal_yTop,
            gb._internal_totalWidth,
            gb._internal_totalHeight,
            labelBackgroundColor,
            gb._internal_horzBorder,
      ...
      if (this._private__data._internal_tickVisible) {
        ...
        ctx.fillRect(
          gb._internal_xInside,
          gb._internal_yMid,
          gb._internal_xTick - gb._internal_xInside,
          gb._internal_tickHeight
        );
      }
      ...
      if (this._private__data._internal_borderVisible) {
        ...
        ctx.fillRect(
          geom._internal_alignRight
            ? gb._internal_right - gb._internal_horzBorder
            : 0,
          gb._internal_yTop,
          gb._internal_horzBorder,
          gb._internal_yBottom - gb._internal_yTop
        );
      }
      return geom;
    });
    target.useMediaCoordinateSpace(({ context: ctx }) => {
      ...
      ctx.fillText(
        ...
        gm._internal_xText,
        (gm._internal_yTop + gm._internal_yBottom) / 2 +
          gm._internal_textMidCorrection
      );
    });
  }
  _private__calculateGeometry(scope, rendererOptions, textWidthCache, align) {
    ...
    const tickSize =
      ...
      !this._private__data._internal_moveTextToInvisibleTick
        ...
        : 0;
    const horzBorder = this._private__data._internal_separatorVisible
      ? rendererOptions._internal_borderSize
      : 0;
    const paddingTop =
      rendererOptions._internal_paddingTop +
      this._private__commonData._internal_additionalPaddingTop;
    const paddingBottom =
      rendererOptions._internal_paddingBottom +
      this._private__commonData._internal_additionalPaddingBottom;
    ...
    const textMidCorrection = textWidthCache._internal_yMidCorrection(
      ctx,
      text
    );
    ...
    const horzBorderBitmap =
      horzBorder > 0
        ? Math.max(1, Math.floor(horzBorder * horizontalPixelRatio))
        : 0;
    ...
    const yMid =
      (_a = this._private__commonData._internal_fixedCoordinate) !== null &&
      _a !== void 0
        ? _a
        : this._private__commonData._internal_coordinate;
    const yMidBitmap =
      Math.round(yMid * verticalPixelRatio) -
      Math.floor(verticalPixelRatio * 0.5);
    const yTopBitmap = Math.floor(
      yMidBitmap + tickHeightBitmap / 2 - totalHeightBitmap / 2
    );
    const yBottomBitmap = yTopBitmap + totalHeightBitmap;
    const alignRight = align === "right";
    const xInside = alignRight ? mediaSize.width - horzBorder : horzBorder;
    const xInsideBitmap = alignRight
      ? bitmapSize.width - horzBorderBitmap
      : horzBorderBitmap;
    let xOutsideBitmap;
    let xTickBitmap;
    let xText;
...
class PriceAxisView {
  ...
  _internal_renderer(priceScale) {
    ...
    this._private__axisRendererData._internal_tickVisible =
      this._private__axisRendererData._internal_tickVisible &&
      priceScale._internal_options().ticksVisible;
...
class CrosshairPriceAxisView extends PriceAxisView {
  ...
  _internal__updateRendererData(
    ...
    const additionalPadding =
      (2 / 12) * this._private__priceScale._internal_fontSize();
...
class TimeAxisViewRenderer {
  ...
  _internal_draw(target, rendererOptions) {
    ...
    const textWidth = target.useMediaCoordinateSpace(({ context: ctx }) => {
      ...
      return Math.round(
        rendererOptions._internal_widthCache._internal_measureText(
          ctx,
          ensureNotNull(this._private__data)._internal_text,
      ...
      ({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) => {
        ...
        if (data._internal_tickVisible) {
          const tickX = Math.round(
            data._internal_coordinate * horizontalPixelRatio
          );
          const tickTop = y1scaled;
          const tickBottom = Math.round(
            (tickTop + rendererOptions._internal_tickLength) *
              verticalPixelRatio
          );
    ...
    target.useMediaCoordinateSpace(({ context: ctx }) => {
      ...
      ctx.textBaseline = "middle";
      ctx.fillStyle = data._internal_color;
      const textYCorrection =
        rendererOptions._internal_widthCache._internal_yMidCorrection(
          ctx,
          "Apr0"
        );
...
class CrosshairTimeAxisView {
  ...
  _private__updateImpl() {
    ...
    const currentTime = timeScale._internal_indexToTimeScalePoint(
...
class Crosshair extends DataSource {
  constructor(model, options) {
    ...
    const valuePriceProvider = (rawPriceProvider, rawCoordinateProvider) => {
      return (priceScale) => {
        ...
        if (
          priceScale ===
          ensureNotNull(this._private__pane)._internal_defaultPriceScale()
        ) {
          ...
          const price = priceScale._internal_coordinateToPrice(
            coordinate,
            firstValue
          );
    ...
    const valueTimeProvider = (rawIndexProvider, rawCoordinateProvider) => {
      return () => {
        const time = this._private__model
          ._internal_timeScale()
    ...
    this._private__currentPosPriceProvider = valuePriceProvider(
      () => this._private__price,
      () => this._private__y
    );
    const currentPosTimeProvider = valueTimeProvider(
      () => this._private__index,
      () => this._internal_appliedX()
    );
    this._private__timeAxisView = new CrosshairTimeAxisView(
      this,
      model,
  ...
  _internal_horzLineVisible(pane) {
    return (
      pane === this._private__pane && this._private__options.horzLine.visible
    );
  }
  ...
  _private__setIndexToLastSeriesBarIndex() {
    const lastIndexes = this._private__model
      ._internal_serieses()
      .map((s) => s._internal_bars()._internal_lastIndex())
      .filter(notNull);
    const lastBarIndex =
      lastIndexes.length === 0 ? null : Math.max(...lastIndexes);
...
function isDefaultPriceScale(priceScaleId) {
  return (
    priceScaleId === "left" /* DefaultPriceScaleId.Left */ ||
    priceScaleId === "right" /* DefaultPriceScaleId.Right */
  );
}
...
class InvalidateMask {
  ...
  _private__removeTimeScaleAnimation() {
    const index = this._private__timeScaleInvalidations.findIndex(
      (inv) =>
        inv._internal_type === 5 /* TimeScaleInvalidationType.Animation */
    );
    if (index !== -1) {
  ...
  _private__formatAsDecimal(price) {
    ...
    const fracLength =
      this._internal__fractionalLength !== undefined
        ...
        : NaN;
    if (base > 1) {
      let fracPart = +(Math.round(price * base) - intPart * base).toFixed(
        this._internal__fractionalLength
      );
      if (fracPart >= base) {
        fracPart -= base;
...
function walkLine(
  ...
  if (
    items.length === 0 ||
    visibleRange.from >= items.length ||
    visibleRange.to <= 0
  ) {
    return;
  }
  ...
  if (visibleRange.to - visibleRange.from < 2) {
    ...
    ctx.moveTo(
      item1._internal_x * horizontalPixelRatio,
      item1._internal_y * verticalPixelRatio
    );
    ctx.lineTo(
      item2._internal_x * horizontalPixelRatio,
      item2._internal_y * verticalPixelRatio
    );
    ...
    ctx.beginPath();
    ctx.moveTo(
      firstItem._internal_x * horizontalPixelRatio,
      firstItem._internal_y * verticalPixelRatio
    );
    for (let i = visibleRange.from + 1; i < visibleRange.to; ++i) {
      ...
      switch (lineType) {
        case 0 /* LineType.Simple */:
          ctx.lineTo(
            currentItem._internal_x * horizontalPixelRatio,
            currentItem._internal_y * verticalPixelRatio
          );
          break;
        case 1 /* LineType.WithSteps */:
          ctx.lineTo(
            currentItem._internal_x * horizontalPixelRatio,
            items[i - 1]._internal_y * verticalPixelRatio
          );
        ...
        case 2 /* LineType.Curved */: {
          ...
          ctx.bezierCurveTo(
            cp1._internal_x * horizontalPixelRatio,
            cp1._internal_y * verticalPixelRatio,
            cp2._internal_x * horizontalPixelRatio,
            cp2._internal_y * verticalPixelRatio,
            currentItem._internal_x * horizontalPixelRatio,
            currentItem._internal_y * verticalPixelRatio
          );
          break;
...
function getControlPoints(points, fromPointIndex, toPointIndex) {
  ...
  const cp1 = add(
    ...
    divide(
      subtract(points[toPointIndex], points[beforeFromPointIndex]),
      curveTension
    )
  );
  const cp2 = subtract(
    points[toPointIndex],
    divide(
      subtract(points[afterToPointIndex], points[fromPointIndex]),
      curveTension
    )
  );
  return [cp1, cp2];
...
function finishStyledArea$1(
  ...
  context.lineTo(
    newAreaFirstItem._internal_x * horizontalPixelRatio,
    baseLevelCoordinate * verticalPixelRatio
  );
  context.lineTo(
    areaFirstItem._internal_x * horizontalPixelRatio,
    baseLevelCoordinate * verticalPixelRatio
  );
  context.closePath();
...
class PaneRendererAreaBase extends BitmapCoordinatesPaneRenderer {
  ...
  _internal__drawImpl(renderingScope) {
    ...
    const baseLevelCoordinate =
      (_a = this._internal__data._internal_baseLevelCoordinate) !== null &&
      _a !== void 0
        ? _a
        : this._internal__data._internal_invertFilledArea
        ? 0
        : renderingScope.mediaSize.height;
    ...
    walkLine(
      ...
      visibleRange,
      barWidth,
      this._internal__fillStyle.bind(this),
      finishStyledArea$1.bind(null, baseLevelCoordinate)
    );
  }
}
...
class GradientStyleCache {
  _internal_get(scope, params) {
    ...
    if (
      this._private__cachedValue === undefined ||
      ...
      if (baseLevelCoordinate != null) {
        const baselinePercent = clamp(
          (baseLevelCoordinate * scope.verticalPixelRatio) / bottom,
          0,
          1
        );
...
function drawSeriesPointMarkers(
  renderingScope,
  items,
  pointMarkersRadius,
  visibleRange,
  ...
  for (let i = visibleRange.to - 1; i >= visibleRange.from; --i) {
    ...
    if (point) {
      ...
      const centerX =
        Math.round(point._internal_x * horizontalPixelRatio) + correction; // correct x coordinate only
...
class SeriesPaneViewBase {
  ...
  _internal_renderer() {
    ...
    return this._internal__itemsVisibleRange === null
      ? null
  ...
  _internal__fillRawPoints() {
    ...
    this._internal__items = this._internal__series
      ...
      .map((row) => {
        ...
        return this._internal__createRawItem(
          row._internal_index,
          value,
          colorer
        );
...
class SeriesAreaPaneView extends LinePaneViewBase {
  ...
  _internal__createRawItem(time, price, colorer) {
    return Object.assign(
      Object.assign({}, this._internal__createRawItemBase(time, price)),
  ...
  _internal__prepareRendererData() {
    ...
    this._private__lineRenderer._internal_setData({
      ...
      _internal_barWidth: this._internal__model
        ._internal_timeScale()
        ._internal_barSpacing(),
      _internal_pointMarkersRadius: options.pointMarkersVisible
...
class PaneRendererBars extends BitmapCoordinatesPaneRenderer {
  ...
  _internal__drawImpl({
    ...
    this._private__barLineWidth = this._private__data._internal_thinBars
      ? Math.min(this._private__barWidth, Math.floor(horizontalPixelRatio))
      : this._private__barWidth;
    let prevColor = null;
    const drawOpenClose =
      this._private__barLineWidth <= this._private__barWidth &&
      this._private__data._internal_barSpacing >=
        Math.floor(1.5 * horizontalPixelRatio);
    for (
      let i = this._private__data._internal_visibleRange.from;
      ...
      if (drawOpenClose) {
        if (this._private__data._internal_openVisible) {
          ...
          let openTop = Math.max(
            bodyTop,
            Math.round(bar._internal_openY * verticalPixelRatio) - bodyWidthHalf
          );
        ...
        let closeTop = Math.max(
          bodyTop,
          Math.round(bar._internal_closeY * verticalPixelRatio) - bodyWidthHalf
        );
...
class BarsPaneViewBase extends SeriesPaneViewBase {
  ...
  _internal__fillRawPoints() {
    ...
    this._internal__items = this._internal__series
      ...
      ._internal_rows()
      .map((row) =>
        this._internal__createRawItem(row._internal_index, row, colorer)
      );
  }
}
...
class SeriesBarsPaneView extends BarsPaneViewBase {
  ...
  _internal__createRawItem(time, bar, colorer) {
    return Object.assign(
      Object.assign({}, this._internal__createDefaultItem(time, bar, colorer)),
      colorer._internal_barStyle(time)
    );
  }
  _internal__prepareRendererData() {
...
class SeriesBaselinePaneView extends LinePaneViewBase {
  ...
  _internal__prepareRendererData() {
    ...
    const options = this._internal__series._internal_options();
    const baseLevelCoordinate = this._internal__series
      ._internal_priceScale()
      ._internal_priceToCoordinate(
        options.baseValue.price,
        firstValue._internal_value
      );
    const barWidth = this._internal__model
      ._internal_timeScale()
      ._internal_barSpacing();
...
class PaneRendererCandlesticks extends BitmapCoordinatesPaneRenderer {
  ...
  _internal__drawImpl(renderingScope) {
    ...
    const borderWidth =
      this._private__calculateBorderWidth(horizontalPixelRatio);
    if (
  ...
  _private__drawWicks(renderingScope, bars, visibleRange) {
    ...
    let wickWidth = Math.min(
      ...
      Math.floor(
        this._private__data._internal_barSpacing * horizontalPixelRatio
      )
    );
    wickWidth = Math.max(
      Math.floor(horizontalPixelRatio),
    ...
    for (let i = visibleRange.from; i < visibleRange.to; i++) {
      ...
      const top = Math.round(
        Math.min(bar._internal_openY, bar._internal_closeY) * verticalPixelRatio
      );
      const bottom = Math.round(
        Math.max(bar._internal_openY, bar._internal_closeY) * verticalPixelRatio
      );
  ...
  _private__calculateBorderWidth(pixelRatio) {
    ...
    if (this._private__barWidth <= res * 2) {
      ...
      return Math.max(
        ...
        Math.floor(1 /* Constants.BarBorderWidth */ * pixelRatio)
      );
    }
    return res;
  }
  _private__drawBorder(renderingScope, bars, visibleRange) {
    ...
    for (let i = visibleRange.from; i < visibleRange.to; i++) {
      ...
      let left =
        Math.round(bar._internal_x * horizontalPixelRatio) -
...
class CustomSeriesPaneRendererWrapper {
  ...
  _internal_draw(target, isHovered, hitTestData) {
    this._private__sourceRenderer.draw(
      target,
...
class SeriesCustomPaneView extends SeriesPaneViewBase {
  constructor(series, model, paneView) {
    ...
    this._internal__renderer = new CustomSeriesPaneRendererWrapper(
      this._private__paneView.renderer(),
      (price) => {
  ...
  _internal__fillRawPoints() {
    ...
    this._internal__items = this._internal__series
      ...
      .map((row) => {
        return Object.assign(
          Object.assign(
            { _internal_time: row._internal_index, _internal_x: NaN },
    ...
    this._private__paneView.update(
      {
        ...
        barSpacing: this._internal__model
          ._internal_timeScale()
          ._internal_barSpacing(),
...
class PaneRendererHistogram extends BitmapCoordinatesPaneRenderer {
  ...
  _internal__drawImpl({
    ...
    const histogramBase = Math.round(
      this._private__data._internal_histogramBase * verticalPixelRatio
    );
  ...
  _private__fillPrecalculatedCache(pixelRatio) {
    ...
    const spacing =
      Math.ceil(this._private__data._internal_barSpacing * pixelRatio) <=
      showSpacingMinimalBarWidth
        ? 0
        : Math.max(1, Math.floor(pixelRatio));
    const columnWidth =
      Math.round(this._private__data._internal_barSpacing * pixelRatio) -
      spacing;
    ...
    // correct positions
    for (
      let i = this._private__data._internal_visibleRange.from + 1;
      i < this._private__data._internal_visibleRange.to;
      i++
    ) {
      const current =
  ...
  _internal__prepareRendererData() {
    const data = {
      ...
      _internal_barSpacing: this._internal__model
        ...
        ._internal_barSpacing(),
      _internal_visibleRange: this._internal__itemsVisibleRange,
      _internal_histogramBase: this._internal__series
        ._internal_priceScale()
        ._internal_priceToCoordinate(
          this._internal__series._internal_options().base,
          ensureNotNull(this._internal__series._internal_firstValue())
            ._internal_value
        ),
    };
...
class TextWidthCache {
  ...
  _internal_measureText(ctx, text, optimizationReplacementRe) {
    return this._private__getMetrics(ctx, text, optimizationReplacementRe)
      .width;
  }
  _internal_yMidCorrection(ctx, text, optimizationReplacementRe) {
    const metrics = this._private__getMetrics(
      ctx,
      text,
      optimizationReplacementRe
    );
  ...
  _private__getMetrics(ctx, text, optimizationReplacementRe) {
    ...
    if (this._private__cache.has(cacheString)) {
      return ensureDefined(this._private__cache.get(cacheString))
        ._internal_metrics;
    }
...
class PanePriceAxisView {
  ...
  _internal_renderer() {
    const pane = this._private__chartModel._internal_paneForSource(
    ...
    const priceScale = pane._internal_isOverlay(this._private__dataSource)
      ? pane._internal_defaultVisiblePriceScale()
...
class HorizontalLineRenderer extends BitmapCoordinatesPaneRenderer {
  ...
  _internal_hitTest(x, y) {
    var _a;
    if (
      !((_a = this._private__data) === null || _a === void 0
        ? void 0
        : _a._internal_visible)
    ) {
      return null;
    }
    const {
    ...
    if (
      y >= itemY - lineWidth - 7 /* Constants.HitTestThreshold */ &&
      y <= itemY + lineWidth + 7 /* Constants.HitTestThreshold */
    ) {
      return {
  ...
  _internal__drawImpl({
    ...
    ctx.lineWidth = Math.floor(
      this._private__data._internal_lineWidth * horizontalPixelRatio
    );
...
class SeriesHorizontalBaseLinePaneView extends SeriesHorizontalLinePaneView {
  ...
  _internal__updateImpl() {
    ...
    if (
      ...
      mode !== 3 /* PriceScaleMode.IndexedTo100 */
    ) {
      return;
    }
...
class SeriesLastPriceAnimationRenderer extends BitmapCoordinatesPaneRenderer {
  ...
  _internal__drawImpl({
    ...
    const correction = (tickWidth % 2) / 2;
    const centerX =
      Math.round(data._internal_center.x * horizontalPixelRatio) + correction; // correct x coordinate only
    const centerY = data._internal_center.y * verticalPixelRatio;
    ctx.fillStyle = data._internal_seriesLineColor;
    ctx.beginPath();
    // TODO: it is better to have different horizontal and vertical radii
    const centerPointRadius =
      Math.max(2, data._internal_seriesLineWidth * 1.5) * horizontalPixelRatio;
    ...
    ctx.arc(
      centerX,
      centerY,
      data._internal_radius * horizontalPixelRatio,
      0,
      2 * Math.PI,
      false
    );
    ...
    ctx.arc(
      centerX,
      centerY,
      data._internal_radius * horizontalPixelRatio + tickWidth / 2,
      0,
      2 * Math.PI,
      false
    );
...
const animationStagesData = [
  ...
  },
  {
    ...
    _internal_end:
      0.25 /* Constants.Stage1Period */ + 0.275 /* Constants.Stage2Period */,
  ...
  },
  {
    _internal_start:
      0.25 /* Constants.Stage1Period */ + 0.275 /* Constants.Stage2Period */,
    _internal_end:
      0.25 /* Constants.Stage1Period */ +
      0.275 /* Constants.Stage2Period */ +
      0.475 /* Constants.Stage3Period */,
...
function animationData(durationSinceStart, lineColor) {
  const globalStage =
    (durationSinceStart % 2600) /* Constants.AnimationPeriod */ /
    2600; /* Constants.AnimationPeriod */
...
class SeriesLastPriceAnimationPaneView {
  ...
  _private__updateImpl() {
    ...
    const timeScale = this._private__series
      ._internal_model()
      ._internal_timeScale();
  ...
  _private__duration() {
    return this._internal_animationActive()
      ...
      : 2600 /* Constants.AnimationPeriod */ - 1;
  }
}
...
function size(barSpacing, coeff) {
  const result =
    Math.min(
      Math.max(barSpacing, 12 /* Constants.MinShapeSize */),
      30 /* Constants.MaxShapeSize */
    ) * coeff;
  ...
  _internal__drawImpl({ context: ctx }, isHovered, hitTestData) {
    ...
    for (
      ...
      if (item._internal_text !== undefined) {
        ...
        item._internal_text._internal_x =
...
function fillSizeAndY(
  ...
  const inBarPrice = isNumber(seriesData)
    ? seriesData
    : seriesData._internal_close;
  const highPrice = isNumber(seriesData)
    ? seriesData
    : seriesData._internal_high;
    ...
    case "inBar": {
      ...
      if (rendererItem._internal_text !== undefined) {
        rendererItem._internal_text._internal_y =
          ...
          halfSize +
          shapeMargin +
          textHeight * (0.5 + 0.1) /* Constants.TextMargin */;
      }
      return;
    }
    case "aboveBar": {
      ...
      if (rendererItem._internal_text !== undefined) {
        rendererItem._internal_text._internal_y =
          ...
          textHeight * (0.5 + 0.1) /* Constants.TextMargin */;
        offsets._internal_aboveBar +=
          textHeight * (1 + 2 * 0.1) /* Constants.TextMargin */;
      }
...
class SeriesMarkersPaneView {
  ...
  _internal_autoScaleMargins() {
    if (this._private__autoScaleMarginsInvalidated) {
      if (this._private__series._internal_indexedMarkers().length > 0) {
        const barSpacing = this._private__model
          ._internal_timeScale()
          ._internal_barSpacing();
  ...
  _internal__makeValid() {
    ...
    for (
      let index = this._private__data._internal_visibleRange.from;
...
class SeriesPriceLinePaneView extends SeriesHorizontalLinePaneView {
  ...
  _internal__updateImpl() {
    ...
    const lastValueData = this._internal__series._internal_lastValueData(
      seriesOptions.priceLineSource === 0 /* PriceLineSource.LastBar */
    );
...
class SeriesPriceAxisView extends PriceAxisView {
  ...
  _internal__updateRendererData(
    ...
    const showPriceAndPercentage =
      ...
      0; /* PriceAxisLastValueMode.LastPriceAndPercentageValue */
    ...
    const lastValueColor = source._internal_priceLineColor(
      lastValueData._internal_color
    );
...
class PriceRangeImpl {
  ...
  _internal_merge(anotherRange) {
    ...
    return new PriceRangeImpl(
      Math.min(this._internal_minValue(), anotherRange._internal_minValue()),
      Math.max(this._internal_maxValue(), anotherRange._internal_maxValue())
    );
  }
...
class CustomPriceLinePriceAxisView extends PriceAxisView {
  ...
  _private__formatPrice(price) {
    ...
    return this._private__series
      ._internal_priceScale()
      ._internal_formatPrice(price, firstValue._internal_value);
  }
}
...
const barStyleFnMap = {
  ...
  Bar: (findBar, barStyle, barIndex, precomputedBars) => {
    ...
    const isUp =
      ensure(currentBar._internal_value[0 /* PlotRowValueIndex.Open */]) <=
      ensure(currentBar._internal_value[3 /* PlotRowValueIndex.Close */]);
    return {
      _internal_barColor:
        (_a = currentBar._internal_color) !== null && _a !== void 0
          ? _a
          : isUp
          ? upColor
          : downColor,
  ...
  Candlestick: (findBar, candlestickStyle, barIndex, precomputedBars) => {
    ...
    return {
      ...
      _internal_barBorderColor:
        (_b = currentBar._internal_borderColor) !== null && _b !== void 0
          ? _b
          : isUp
          ? borderUpColor
          : borderDownColor,
      _internal_barWickColor:
        (_c = currentBar._internal_wickColor) !== null && _c !== void 0
          ? _c
          : isUp
          ? wickUpColor
          : wickDownColor,
  ...
  Area: (findBar, areaStyle, barIndex, precomputedBars) => {
    ...
    return {
      _internal_barColor:
        (_a = currentBar._internal_lineColor) !== null && _a !== void 0
          ? _a
          : areaStyle.lineColor,
      _internal_lineColor:
        (_b = currentBar._internal_lineColor) !== null && _b !== void 0
          ? _b
          : areaStyle.lineColor,
      _internal_topColor:
        (_c = currentBar._internal_topColor) !== null && _c !== void 0
          ? _c
          : areaStyle.topColor,
      _internal_bottomColor:
        (_d = currentBar._internal_bottomColor) !== null && _d !== void 0
          ? _d
          : areaStyle.bottomColor,
    };
  },
  // eslint-disable-next-line @typescript-eslint/naming-convention
  Baseline: (findBar, baselineStyle, barIndex, precomputedBars) => {
    ...
    const isAboveBaseline =
      currentBar._internal_value[3 /* PlotRowValueIndex.Close */] >=
      baselineStyle.baseValue.price;
    return {
      _internal_barColor: isAboveBaseline
        ? baselineStyle.topLineColor
        : baselineStyle.bottomLineColor,
      _internal_topLineColor:
        (_a = currentBar._internal_topLineColor) !== null && _a !== void 0
          ? _a
          : baselineStyle.topLineColor,
      _internal_bottomLineColor:
        (_b = currentBar._internal_bottomLineColor) !== null && _b !== void 0
          ? _b
          : baselineStyle.bottomLineColor,
      _internal_topFillColor1:
        (_c = currentBar._internal_topFillColor1) !== null && _c !== void 0
          ? _c
          : baselineStyle.topFillColor1,
      _internal_topFillColor2:
        (_d = currentBar._internal_topFillColor2) !== null && _d !== void 0
          ? _d
          : baselineStyle.topFillColor2,
      _internal_bottomFillColor1:
        (_e = currentBar._internal_bottomFillColor1) !== null && _e !== void 0
          ? _e
          : baselineStyle.bottomFillColor1,
      _internal_bottomFillColor2:
        (_f = currentBar._internal_bottomFillColor2) !== null && _f !== void 0
          ? _f
          : baselineStyle.bottomFillColor2,
    };
  },
  // eslint-disable-next-line @typescript-eslint/naming-convention
    ...
    return {
      _internal_barColor:
        (_a = currentBar._internal_color) !== null && _a !== void 0
          ...
          : lineStyle.color,
      _internal_lineColor:
        (_b = currentBar._internal_color) !== null && _b !== void 0
          ? _b
          : lineStyle.color,
    };
  },
...
class SeriesBarColorer {
  ...
  _internal_barStyle(barIndex, precomputedBars) {
    ...
    return this._private__styleGetter(
      this._private__findBar,
      this._private__series._internal_options(),
      barIndex,
      precomputedBars
    );
  }
...
class PlotList {
  ...
  _internal_contains(index) {
    return (
      this._private__search(index, 0 /* MismatchDirection.None */) !== null
    );
  }
  ...
  _internal_minMaxOnRangeCached(start, end, plots) {
    ...
    for (const plot of plots) {
      const plotMinMax = this._private__minMaxOnRangeCachedImpl(
        start,
        end,
        plot
      );
  ...
  _private__minMaxOnRangeCachedImpl(start, end, plotIndex) {
    ...
    );
    {
      ...
      const plotMinMax = this._private__plotMinMax(
        startIndex,
        endIndex,
        plotIndex
      );
    ...
    for (let c = Math.max(cachedLow + 1, s); c < cachedHigh; c += CHUNK_SIZE) {
      ...
      if (chunkMinMax === undefined) {
        ...
        const chunkEnd = this._private__upperbound(
...
class SeriesPrimitiveRendererWrapper {
  ...
  _internal_drawBackground(target, isHovered, hitTestData) {
    var _a, _b;
    (_b = (_a = this._private__baseRenderer).drawBackground) === null ||
    _b === void 0
      ? void 0
      : _b.call(_a, target);
  }
...
class SeriesPrimitivePaneViewWrapper {
  ...
  _internal_renderer() {
    ...
    if (
      ((_a = this._private__cache) === null || _a === void 0
        ? void 0
        : _a._internal_base) === baseRenderer
    ) {
  ...
  _internal_zOrder() {
    var _a, _b, _c;
    return (_c =
      (_b = (_a = this._private__paneView).zOrder) === null || _b === void 0
        ? void 0
...
function getAxisViewData(baseView) {
  ...
  return {
    ...
    _internal_fixedCoordinate:
      (_a = baseView.fixedCoordinate) === null || _a === void 0
        ? void 0
        : _a.call(baseView),
    ...
    _internal_visible:
      (_c =
        (_b = baseView.visible) === null || _b === void 0
          ? void 0
          : _b.call(baseView)) !== null && _c !== void 0
        ? _c
        : true,
    _internal_tickVisible:
      (_e =
        (_d = baseView.tickVisible) === null || _d === void 0
          ? void 0
          : _d.call(baseView)) !== null && _e !== void 0
        ? _e
        : true,
  };
}
...
class SeriesPrimitivePriceAxisViewWrapper extends PriceAxisView {
  ...
  _internal__updateRendererData(
    ...
    commonRendererData._internal_fixedCoordinate =
      data._internal_fixedCoordinate;
...
class SeriesPrimitiveWrapper {
  ...
  _internal_updateAllViews() {
    var _a, _b;
    (_b = (_a = this._private__primitive).updateAllViews) === null ||
    _b === void 0
      ? void 0
      : _b.call(_a);
  }
  _internal_paneViews() {
    ...
    const base =
      (_c =
        (_b = (_a = this._private__primitive).paneViews) === null ||
        _b === void 0
          ? void 0
          : _b.call(_a)) !== null && _c !== void 0
        ? _c
        : [];
    if (
      ((_d = this._private__paneViewsCache) === null || _d === void 0
        ? void 0
        : _d._internal_base) === base
    ) {
  ...
  _internal_timeAxisViews() {
    ...
    const base =
      (_c =
        (_b = (_a = this._private__primitive).timeAxisViews) === null ||
        _b === void 0
          ? void 0
          : _b.call(_a)) !== null && _c !== void 0
        ? _c
        : [];
    if (
      ((_d = this._private__timeAxisViewsCache) === null || _d === void 0
        ? void 0
        : _d._internal_base) === base
    ) {
    ...
    const wrapper = base.map(
      (aw) => new SeriesPrimitiveTimeAxisViewWrapper(aw, timeScale)
    );
  ...
  _internal_priceAxisViews() {
    ...
    const base =
      (_c =
        (_b = (_a = this._private__primitive).priceAxisViews) === null ||
        _b === void 0
          ? void 0
          : _b.call(_a)) !== null && _c !== void 0
        ? _c
        : [];
    if (
      ((_d = this._private__priceAxisViewsCache) === null || _d === void 0
        ? void 0
        : _d._internal_base) === base
    ) {
    ...
    const priceScale = this._private__series._internal_priceScale();
    const wrapper = base.map(
      (aw) => new SeriesPrimitivePriceAxisViewWrapper(aw, priceScale)
    );
  ...
  _internal_priceAxisPaneViews() {
    ...
    const base =
      (_c =
        (_b = (_a = this._private__primitive).priceAxisPaneViews) === null ||
        _b === void 0
          ? void 0
          : _b.call(_a)) !== null && _c !== void 0
        ? _c
        : [];
    if (
      ((_d = this._private__priceAxisPaneViewsCache) === null || _d === void 0
        ? void 0
        : _d._internal_base) === base
    ) {
  ...
  _internal_timeAxisPaneViews() {
    ...
    const base =
      (_c =
        (_b = (_a = this._private__primitive).timeAxisPaneViews) === null ||
        _b === void 0
          ? void 0
          : _b.call(_a)) !== null && _c !== void 0
        ? _c
        : [];
    if (
      ((_d = this._private__timeAxisPaneViewsCache) === null || _d === void 0
        ? void 0
        : _d._internal_base) === base
    ) {
  ...
  _internal_autoscaleInfo(startTimePoint, endTimePoint) {
    var _a, _b, _c;
    return (_c =
      (_b = (_a = this._private__primitive).autoscaleInfo) === null ||
      _b === void 0
        ? void 0
        : _b.call(_a, startTimePoint, endTimePoint)) !== null && _c !== void 0
      ? _c
      : null;
  }
  _internal_hitTest(x, y) {
    var _a, _b, _c;
    return (_c =
      (_b = (_a = this._private__primitive).hitTest) === null || _b === void 0
        ? void 0
        : _b.call(_a, x, y)) !== null && _c !== void 0
      ? _c
      : null;
  }
}
...
class Series extends PriceDataSource {
  constructor(model, options, seriesType, pane, customPaneView) {
    ...
    this._private__baseHorizontalLineView =
    ...
    if (
      ...
      this._private__lastPriceAnimationPaneView =
        new SeriesLastPriceAnimationPaneView(this);
    }
  ...
  _internal_lastValueData(globalLast) {
    ...
    const visibleBars = this._internal_model()
      ._internal_timeScale()
    ...
    const coordinate = priceScale._internal_priceToCoordinate(
      price,
      firstValue._internal_value
    );
    return {
  ...
  _internal_setData(data, updateInfo) {
    ...
    if (this._private__lastPriceAnimationPaneView !== null) {
      if (
        updateInfo &&
        updateInfo._internal_lastBarUpdatedOrNewBarsAddedToTheRight
      ) {
  ...
  _internal_dataAt(time) {
    ...
    if (
      ...
      this._private__seriesType === "Custom"
    ) {
      return {
        ...
        _internal_close:
          prices._internal_value[3 /* PlotRowValueIndex.Close */],
      };
    } else {
  ...
  _internal_paneViews() {
    ...
    const priceLineViews = this._private__customPriceLines.map((line) =>
      line._internal_paneView()
    );
  ...
  _internal_bottomPaneViews() {
    return this._private__extractPaneViews(
      primitivePaneViewsExtractor,
      "bottom"
    );
  }
  _internal_pricePaneViews(zOrder) {
    return this._private__extractPaneViews(
      primitivePricePaneViewsExtractor,
      zOrder
    );
  }
  _internal_timePaneViews(zOrder) {
    return this._private__extractPaneViews(
      primitiveTimePaneViewsExtractor,
      zOrder
    );
  }
  ...
  _internal_labelPaneViews(pane) {
    return [
      ...
      ...this._private__customPriceLines.map((line) =>
        line._internal_labelPaneView()
      ),
    ];
  }
  _internal_priceAxisViews(pane, priceScale) {
  ...
  _internal_autoscaleInfo(startTimePoint, endTimePoint) {
    if (this._private__options.autoscaleInfoProvider !== undefined) {
      const autoscaleInfo = this._private__options.autoscaleInfoProvider(() => {
        const res = this._private__autoscaleInfoImpl(
          startTimePoint,
          endTimePoint
        );
  ...
  _internal_updateAllViews() {
    ...
    (_a = this._private__lastPriceAnimationPaneView) === null || _a === void 0
      ? void 0
      : _a._internal_update();
    this._private__primitives.forEach((wrapper) =>
      wrapper._internal_updateAllViews()
    );
  }
  _internal_priceScale() {
  ...
  _internal_detachPrimitive(source) {
    this._private__primitives = this._private__primitives.filter(
      (wrapper) => wrapper._internal_primitive() !== source
    );
  }
  ...
  _private__autoscaleInfoImpl(startTimePoint, endTimePoint) {
    ...
    const barsMinMax = this._private__data._internal_minMaxOnRangeCached(
      startTimePoint,
      endTimePoint,
      plots
    );
    let range =
      barsMinMax !== null
        ? new PriceRangeImpl(barsMinMax._internal_min, barsMinMax._internal_max)
        : null;
    if (this._internal_seriesType() === "Histogram") {
      const base = this._private__options.base;
      const rangeWithBase = new PriceRangeImpl(base, base);
      range =
        range !== null ? range._internal_merge(rangeWithBase) : rangeWithBase;
    }
    let margins = this._private__markersPaneView._internal_autoScaleMargins();
    this._private__primitives.forEach((primitive) => {
      const primitiveAutoscale = primitive._internal_autoscaleInfo(
        startTimePoint,
        endTimePoint
      );
      if (
        primitiveAutoscale === null || primitiveAutoscale === void 0
          ? void 0
          : primitiveAutoscale.priceRange
      ) {
        ...
        range =
          range !== null
            ? range._internal_merge(primitiveRange)
            : primitiveRange;
      }
      if (
        primitiveAutoscale === null || primitiveAutoscale === void 0
          ? void 0
          : primitiveAutoscale.margins
      ) {
  ...
  _private__recalculateMarkers() {
    ...
    if (
      !timeScale._internal_hasPoints() ||
      this._private__data._internal_isEmpty()
    ) {
      this._private__indexedMarkers = [];
      return;
    }
    const firstDataIndex = ensureNotNull(
      this._private__data._internal_firstIndex()
    );
    this._private__indexedMarkers = this._private__markers.map(
      (marker, index) => {
        // the first find index on the time scale (across all series)
        const timePointIndex = ensureNotNull(
        ...
        const searchMode =
          timePointIndex < firstDataIndex
            ? 1 /* MismatchDirection.NearestRight */
            : -1; /* MismatchDirection.NearestLeft */
        const seriesDataIndex = ensureNotNull(
          this._private__data._internal_search(timePointIndex, searchMode)
        )._internal_index;
        return {
  ...
  _private__recreatePaneViews(customPaneView) {
    ...
    switch (this._private__seriesType) {
      ...
      case "Custom": {
        this._private__paneView = new SeriesCustomPaneView(
          this,
          this._internal_model(),
...
function mergeMargins(source, additionalMargin) {
  ...
  return {
    above: Math.max(
      (_a = source === null || source === void 0 ? void 0 : source.above) !==
        null && _a !== void 0
        ? _a
        : 0,
      additionalMargin.above
    ),
    below: Math.max(
      (_b = source === null || source === void 0 ? void 0 : source.below) !==
        null && _b !== void 0
        ? _b
        : 0,
      additionalMargin.below
    ),
  };
}
...
class Magnet {
  ...
  _internal_align(price, index, pane) {
    ...
    const serieses = pane
      ...
      .filter((ds) => ds instanceof Series);
    const candidates = serieses.reduce((acc, series) => {
      ...
      return acc.concat([
        ps._internal_priceToCoordinate(
          bar._internal_value[3 /* PlotRowValueIndex.Close */],
...
class GridPaneView {
  ...
  _internal_renderer() {
    if (this._private__invalidated) {
      const gridOptions = this._private__pane
        ._internal_model()
...
function toLog(price, logFormula) {
  ...
  const res =
    log10(m + logFormula._internal_coordOffset) +
    logFormula._internal_logicalOffset;
...
function fromLog(logical, logFormula) {
  ...
  const res =
    Math.pow(10, m - logFormula._internal_logicalOffset) -
    logFormula._internal_coordOffset;
...
function logFormulaForPriceRange(range) {
  ...
  const diff = Math.abs(
    range._internal_maxValue() - range._internal_minValue()
  );
...
function logFormulasAreSame(f1, f2) {
  return (
    f1._internal_logicalOffset === f2._internal_logicalOffset &&
    f1._internal_coordOffset === f2._internal_coordOffset
  );
}
...
class PriceTickSpanCalculator {
  ...
  _internal_tickSpan(high, low, maxTickSpan) {
    ...
    while (true) {
      ...
      const resultTickSpanLargerMinMovement =
        greaterOrEqual(
          resultTickSpan,
          minMovement,
          1e-14 /* Constants.TickSpanEpsilon */
        ) &&
        resultTickSpan > minMovement + 1e-14; /* Constants.TickSpanEpsilon */
    ...
    if (
      ...
      equal(resultTickSpan, 1, 1e-14 /* Constants.TickSpanEpsilon */)
    ) {
      index = 0;
...
class PriceTickMarkBuilder {
  ...
  _internal_tickSpan(high, low) {
    ...
    const spanCalculator1 = new PriceTickSpanCalculator(
      this._private__base,
      [2, 2.5, 2]
    );
    const spanCalculator2 = new PriceTickSpanCalculator(
      this._private__base,
      [2, 2, 2.5]
    );
    const spanCalculator3 = new PriceTickSpanCalculator(
      this._private__base,
      [2.5, 2, 2]
    );
    const spans = [];
  ...
  _internal_rebuildTickMarks() {
    ...
    const bottom = this._private__coordinateToLogicalFunc(
      scaleHeight - 1,
      firstValue
    );
    const top = this._private__coordinateToLogicalFunc(0, firstValue);
    const extraTopBottomMargin = this._private__priceScale._internal_options()
      .entireTextOnly
    ...
    for (let logical = high - mod; logical > low; logical -= span) {
      const coord = this._private__logicalToCoordinateFunc(
        logical,
        firstValue,
        true
      );
...
class PriceScale {
  ...
  _internal_setMode(newMode) {
    ...
    if (newMode._internal_mode !== undefined) {
      ...
      if (
        newMode._internal_mode === 2 /* PriceScaleMode.Percentage */ ||
        newMode._internal_mode === 3 /* PriceScaleMode.IndexedTo100 */
      ) {
    ...
    if (
      newMode._internal_mode === 1 /* PriceScaleMode.Logarithmic */ &&
      newMode._internal_mode !== oldMode._internal_mode
    ) {
      priceRange = convertPriceRangeToLog(
        this._private__priceRange,
        this._private__logFormula
      );
      if (priceRange !== null) {
    ...
    if (
      modeChanged &&
      (oldMode._internal_mode === 2 /* PriceScaleMode.Percentage */ ||
    ...
    if (
      modeChanged &&
      (oldMode._internal_mode === 3 /* PriceScaleMode.IndexedTo100 */ ||
  ...
  _internal_marks() {
    ...
    if (
      this._private__marksCache !== null &&
      (firstValueIsNull ||
        this._private__marksCache._internal_firstValueIsNull ===
          firstValueIsNull)
    ) {
  ...
  _internal_scaleTo(x) {
    ...
    let scaleCoeff =
      (this._private__scaleStartPoint + (this._private__height - 1) * 0.2) /
      (x + (this._private__height - 1) * 0.2);
    const newPriceRange = ensureNotNull(
  ...
  _internal_formatPriceAbsolute(price) {
    return this._private__formatPrice(
      price,
      ensureNotNull(this._private__formatterSource())._internal_formatter()
    );
  }
  ...
  _private__topMarginPx() {
    return this._internal_isInverted()
      ? this._private__options.scaleMargins.bottom * this._internal_height() +
          this._private__marginBelow
      : this._private__options.scaleMargins.top * this._internal_height() +
  ...
  _private__logicalToCoordinate(logical, baseValue) {
    ...
    logical =
      this._internal_isLog() && logical
        ? toLog(logical, this._private__logFormula)
        : logical;
    const range = ensureNotNull(this._internal_priceRange());
    const invCoordinate =
      this._private__bottomMarginPx() +
    ...
    const logical =
      ...
      range._internal_length() *
        ((invCoordinate - this._private__bottomMarginPx()) /
          (this._internal_internalHeight() - 1));
    return this._internal_isLog()
      ? fromLog(logical, this._private__logFormula)
      : logical;
  }
  ...
  _private__recalculatePriceRangeImpl() {
    ...
    for (const source of sources) {
      ...
      const autoScaleInfo = source._internal_autoscaleInfo(
      ...
      if (sourceRange !== null) {
        switch (this._private__options.mode) {
          case 1 /* PriceScaleMode.Logarithmic */:
            sourceRange = convertPriceRangeToLog(
              sourceRange,
              this._private__logFormula
            );
            break;
    ...
    if (priceRange !== null) {
      ...
      if (this._internal_isLog()) {
        const rawRange = convertPriceRangeFromLog(
          priceRange,
          this._private__logFormula
        );
        ...
        if (!logFormulasAreSame(newLogFormula, this._private__logFormula)) {
          ...
          if (rawSnapshot !== null) {
            this._private__priceRangeSnapshot = convertPriceRangeToLog(
              rawSnapshot,
              newLogFormula
            );
          }
  ...
  _private__formatPrice(price, fallbackFormatter) {
    return this._private__formatValue(
      price,
      this._private__localizationOptions.priceFormatter,
      fallbackFormatter
    );
  }
  _private__formatPercentage(percentage, fallbackFormatter) {
    return this._private__formatValue(
      percentage,
      this._private__localizationOptions.percentageFormatter,
      fallbackFormatter
    );
  }
}
...
const MMMM = (date, locale) =>
  new Date(date.getUTCFullYear(), date.getUTCMonth(), 1).toLocaleString(
    locale,
    { month: "long" }
  );
const MMM = (date, locale) =>
  new Date(date.getUTCFullYear(), date.getUTCMonth(), 1).toLocaleString(
    locale,
    { month: "short" }
  );
...
class DateTimeFormatter {
  constructor(params = {}) {
    const formatterParams = Object.assign(
  ...
  _internal_format(dateTime) {
    return `${this._private__dateFormatter._internal_format(dateTime)}${
...
class FormattedLabelsCache {
  ...
  _internal_format(tickMark) {
    ...
    const cacheKey =
      time._internal_businessDay === undefined
        ? new Date(time._internal_timestamp * 1000).getTime()
        : new Date(
            Date.UTC(
              time._internal_businessDay.year,
              time._internal_businessDay.month - 1,
              time._internal_businessDay.day
            )
          ).getTime();
    ...
    if (this._private__actualSize === this._private__maxSize) {
      const oldestValue = this._private__tick2Labels.get(
        this._private__oldestTick
      );
...
class TickMarks {
  ...
  _internal_setTimeScalePoints(newPoints, firstChangedPointIndex) {
    ...
    for (
      ...
      let marksForWeight = this._private__marksByWeight.get(
      ...
      if (sinceIndex <= marks[0]._internal_index) {
        ...
        marks.splice(
          lowerbound(
            marks,
            sinceIndex,
            (tm) => tm._internal_index < sinceIndex
          ),
          Infinity
        );
      }
  ...
  _private__buildMarksImpl(maxIndexesPerMark) {
    let marks = [];
    for (const weight of Array.from(this._private__marksByWeight.keys()).sort(
      (a, b) => b - a
    )) {
      ...
      const currentWeight = ensureDefined(
...
class TimeScale {
  ...
  _internal_indexToTime(index) {
    ...
    return (_b =
      (_a = this._private__points[index]) === null || _a === void 0
        ? void 0
        : _a._internal_time) !== null && _b !== void 0
      ? _b
      : null;
  }
  _internal_indexToTimeScalePoint(index) {
    var _a;
    return (_a = this._private__points[index]) !== null && _a !== void 0
      ? _a
      : null;
  }
  _internal_timeToIndex(time, findNearest) {
    ...
    if (
      ...
      this._private__points[this._private__points.length - 1]._internal_time
        ._internal_timestamp
    ) {
      // special case
    ...
    const index = lowerbound(
      this._private__points,
      time._internal_timestamp,
      (a, b) => a._internal_time._internal_timestamp < b
    );
    if (
      time._internal_timestamp <
  ...
  _internal_setWidth(newWidth) {
    ...
    if (this._private__options.fixLeftEdge) {
      ...
      if (
        ...
        previousVisibleRange._internal_left() <= 0
      ) {
  ...
  _internal_marks() {
    ...
    const items = this._private__tickMarks._internal_build(
      spacing,
      maxLabelWidth
    );
    ...
    const isAllScalingAndScrollingDisabled =
      this._private__isAllScalingAndScrollingDisabled();
    const isLeftEdgeFixed =
      this._private__options.fixLeftEdge || isAllScalingAndScrollingDisabled;
    const isRightEdgeFixed =
      this._private__options.fixRightEdge || isAllScalingAndScrollingDisabled;
    let targetIndex = 0;
    for (const tm of items) {
      ...
      if (
        this._private__barSpacing > maxLabelWidth / 2 &&
  ...
  _internal_scaleTo(x) {
    ...
    const startLengthFromRight = clamp(
      this._private__width - x,
      0,
      this._private__width
    );
    const currentLengthFromRight = clamp(
      this._private__width - ensureNotNull(this._private__scaleStartPoint),
      0,
      this._private__width
    );
  ...
  _internal_scrollToOffsetAnimated(
    offset,
    animationDuration = 400 /* Constants.DefaultAnimationDuration */
  ) {
    if (!isFinite(offset)) {
    ...
    this._private__model._internal_setTimeScaleAnimation({
      _internal_finished: (time) =>
        (time - animationStart) / animationDuration >= 1,
  ...
  _internal_formatDateTime(timeScalePoint) {
    if (this._private__localizationOptions.timeFormatter !== undefined) {
      return this._private__localizationOptions.timeFormatter(
        timeScalePoint._internal_originalTime
      );
    }
    return this._private__dateTimeFormatter._internal_format(
      new Date(timeScalePoint._internal_time._internal_timestamp * 1000)
    );
  }
  _private__isAllScalingAndScrollingDisabled() {
    const { handleScroll, handleScale } =
  ...
  _private__minRightOffset() {
    ...
    const barsEstimation = this._private__options.fixLeftEdge
      ...
      : Math.min(
          2 /* Constants.MinVisibleBarsCount */,
  ...
  _private__formatLabel(tickMark) {
    let formatter = this._private__formattedByWeight.get(
      tickMark._internal_weight
    );
    if (formatter === undefined) {
      formatter = new FormattedLabelsCache((mark) => {
        return this._private__formatLabelImpl(mark);
      });
      this._private__formattedByWeight.set(
        tickMark._internal_weight,
        formatter
      );
    }
  ...
  _private__formatLabelImpl(tickMark) {
    ...
    if (this._private__options.tickMarkFormatter !== undefined) {
      const tickMarkString = this._private__options.tickMarkFormatter(
        tickMark._internal_originalTime,
        tickMarkType,
...
function weightToTickMarkType(weight, timeVisible, secondsVisible) {
  switch (weight) {
    ...
    case 10 /* TickMarkWeight.Second */:
      return timeVisible
        ? secondsVisible
          ? 4 /* TickMarkType.TimeWithSeconds */
          : 3 /* TickMarkType.Time */
...
class ChartModel {
  ...
  _internal_cursorUpdate() {
    this._private__invalidate(
      new InvalidateMask(1 /* InvalidationLevel.Cursor */)
    );
  }
  ...
  _internal_scalePriceTo(pane, priceScale, x) {
    ...
    this._private__invalidate(
      this._private__paneInvalidationMask(pane, 2 /* InvalidationLevel.Light */)
    );
  }
  ...
  _internal_updateTimeScale(newBaseIndex, newPoints, firstChangedPointIndex) {
    ...
    if (
      ...
      const isLastSeriesBarVisible =
        visibleBars._internal_contains(currentBaseIndex);
      const isLeftBarShiftToLeft =
        oldFirstTime._internal_timestamp > newFirstTime._internal_timestamp;
      ...
      if (isSeriesPointsAddedToRight && !needShiftVisibleRangeOnNewBar) {
        ...
        this._private__timeScale._internal_setRightOffset(
          this._private__timeScale._internal_rightOffset() - compensationShift
        );
      }
    }
  ...
  _internal_paneForSource(source) {
    const pane = this._private__panes.find((p) =>
      p._internal_orderedSources().includes(source)
    );
  ...
  _internal_createSeries(seriesType, options, customPaneView) {
    ...
    const series = this._private__createSeries(
      options,
      seriesType,
      pane,
      customPaneView
  ...
  _internal_moveSeriesToScale(series, targetScaleId) {
    ...
    if (target === null) {
      ...
      // if move to new pane
      const zOrder =
        target._internal_pane === pane ? series._internal_zorder() : undefined;
  ...
  _private__invalidate(mask) {
    ...
    this._private__panes.forEach((pane) =>
      pane._internal_grid()._internal_paneView()._internal_update()
    );
  }
  ...
  if (
    (_a =
      navigator === null || navigator === void 0
        ? void 0
        : navigator.userAgentData) === null || _a === void 0
      ? void 0
      : _a.platform
  ) {
...
function durationMSec(speed, dumpingCoeff) {
  ...
  return (
    Math.log((1 /* Constants.EpsilonDistance */ * lnDumpingCoeff) / -speed) /
    lnDumpingCoeff
  );
}
class KineticAnimation {
  ...
  _internal_addPosition(position, time) {
    if (this._private__position1 !== null) {
      ...
      if (
        Math.abs(this._private__position1._internal_position - position) <
        this._private__minMove
      ) {
        return;
      }
    ...
    if (this._private__position3 !== null) {
      ...
      if (Math.sign(speed2) === Math.sign(speed1)) {
        ...
        if (this._private__position4 !== null) {
          const speed3 = speedPxPerMSec(
            this._private__position3,
            this._private__position4,
            this._private__maxSpeed
          );
          if (Math.sign(speed3) === Math.sign(speed1)) {
            const distance3 = distanceBetweenPoints(
              this._private__position3,
              this._private__position4
            );
  ...
  _internal_finished(time) {
    return (
      this._private__animationStartPosition === null ||
      this._private__progressDuration(time) === this._private__durationMsecs
    );
  }
...
class MouseEventHandler {
    ...
    this._private__onMobileSafariDoubleClick = (dblClickEvent) => {
      if (this._private__firesTouchEvents(dblClickEvent)) {
        ...
        if (this._private__tapTimeoutId && this._private__tapCount > 1) {
          const { _internal_manhattanDistance: manhattanDistance } =
            this._private__touchMouseMoveWithDownInfo(
              getPosition(dblClickEvent),
              this._private__tapPosition
            );
          if (
            manhattanDistance < 30 /* Constants.DoubleTapManhattanDistance */ &&
            !this._private__cancelTap
          ) {
            this._private__processTouchEvent(
              compatEvent,
              this._private__handler._internal_doubleTapEvent
            );
          }
        ...
        if (this._private__clickTimeoutId && this._private__clickCount > 1) {
          ...
          if (
            manhattanDistance <
              5 /* Constants.DoubleClickManhattanDistance */ &&
            !this._private__cancelClick
          ) {
            this._private__processMouseEvent(
              compatEvent,
  ...
  _private__mouseEnterHandler(enterEvent) {
    ...
    this._private__unsubscribeMousemove = () => {
      this._private__target.removeEventListener(
        "mousemove",
  ...
  _private__touchMoveHandler(moveEvent) {
    const touch = touchWithId(
      moveEvent.changedTouches,
      ensureNotNull(this._private__activeTouchId)
    );
    if (touch === null) {
      return;
    }
    ...
    const moveInfo = this._private__touchMouseMoveWithDownInfo(
      getPosition(touch),
      ensureNotNull(this._private__touchMoveStartPosition)
    );
    const {
    ...
    if (
      !this._private__touchMoveExceededManhattanDistance &&
      manhattanDistance < 5 /* Constants.CancelTapManhattanDistance */
    ) {
      return;
    }
    ...
    if (!this._private__preventTouchDragProcess) {
      ...
      this._private__processTouchEvent(
        compatEvent,
        this._private__handler._internal_touchMoveEvent
      );
  ...
  _private__mouseMoveWithDownHandler(moveEvent) {
    ...
    const moveInfo = this._private__touchMouseMoveWithDownInfo(
      ...
      ensureNotNull(this._private__mouseMoveStartPosition)
    );
  ...
  _private__touchMouseMoveWithDownInfo(currentPosition, startPosition) {
    const xOffset = Math.abs(
      startPosition._internal_x - currentPosition._internal_x
    );
    const yOffset = Math.abs(
      startPosition._internal_y - currentPosition._internal_y
    );
  ...
  _private__touchEndHandler(touchEndEvent) {
    ...
    this._private__processTouchEvent(
      compatEvent,
      this._private__handler._internal_touchEndEvent
    );
    ++this._private__tapCount;
    if (this._private__tapTimeoutId && this._private__tapCount > 1) {
      ...
      if (!this._private__cancelTap) {
        this._private__processTouchEvent(
          compatEvent,
          this._private__handler._internal_tapEvent
        );
  ...
  _private__mouseUpHandler(mouseUpEvent) {
    ...
    if (isFF()) {
      ...
      rootElement.removeEventListener(
        "mouseleave",
        this._private__onFirefoxOutsideMouseUp
      );
    }
    ...
    if (this._private__clickTimeoutId && this._private__clickCount > 1) {
      ...
      if (
        manhattanDistance < 5 /* Constants.DoubleClickManhattanDistance */ &&
        !this._private__cancelClick
      ) {
        this._private__processMouseEvent(
          compatEvent,
  ...
  _private__touchStartHandler(downEvent) {
    ...
    {
      const boundTouchMoveWithDownHandler =
        this._private__touchMoveHandler.bind(this);
  ...
  _private__init() {
    ...
    );
    {
      ...
      const outsideHandler = (event) => {
        ...
        if (
          event.composed &&
          this._private__target.contains(event.composedPath()[0])
        ) {
          return;
        }
    ...
    if (isIOS()) {
      this._private__unsubscribeMobileSafariEvents = () => {
        this._private__target.removeEventListener(
          "dblclick",
          this._private__onMobileSafariDoubleClick
        );
      };
      this._private__target.addEventListener(
        "dblclick",
    ...
    this._private__target.addEventListener(
      "touchstart",
      this._private__touchStartHandler.bind(this),
      { passive: true }
    );
  ...
  _private__initPinch() {
    ...
    this._private__target.addEventListener(
      "touchstart",
      (event) => this._private__checkPinchState(event.touches),
      { passive: true }
    );
    this._private__target.addEventListener(
      "touchmove",
      (event) => {
        ...
        if (this._private__handler._internal_pinchEvent !== undefined) {
          ...
          this._private__handler._internal_pinchEvent(
  ...
  _private__startPinch(touches) {
    ...
    this._private__startPinchMiddlePoint = {
      _internal_x:
        (touches[0].clientX - box.left + (touches[1].clientX - box.left)) / 2,
      _internal_y:
        (touches[0].clientY - box.top + (touches[1].clientY - box.top)) / 2,
    };
  ...
  _private__firesTouchEvents(e) {
    if (
      ...
      e.sourceCapabilities.firesTouchEvents !== undefined
    ) {
      return e.sourceCapabilities.firesTouchEvents;
    }
    return (
      eventTimeStamp(e) <
      this._private__lastTouchEventTimeStamp +
        500 /* Delay.PreventFiresTouchEvents */
    );
  }
...
function findBestPrimitiveHitTest(sources, x, y) {
  ...
  let bestHitSource;
  for (const source of sources) {
    const primitiveHitResults =
      (_b =
        (_a = source._internal_primitiveHitTest) === null || _a === void 0
          ? void 0
          : _a.call(source, x, y)) !== null && _b !== void 0
        ? _b
        : [];
    for (const hitResult of primitiveHitResults) {
      if (
        comparePrimitiveZOrder(
          hitResult.zOrder,
          bestPrimitiveHit === null || bestPrimitiveHit === void 0
            ? void 0
            : bestPrimitiveHit.zOrder
        )
      ) {
...
function hitTestPane(pane, x, y) {
  ...
  if (
    (bestPrimitiveHit === null || bestPrimitiveHit === void 0
      ? void 0
      : bestPrimitiveHit._internal_hit.zOrder) === "top"
  ) {
  ...
  for (const source of sources) {
    if (
      bestPrimitiveHit &&
      bestPrimitiveHit._internal_source === source &&
      bestPrimitiveHit._internal_hit.zOrder !== "bottom" &&
      !bestPrimitiveHit._internal_hit.isBackground
    ) {
      // a primitive will be drawn above a built-in item like a series marker
      // therefore it takes precedence here.
      return convertPrimitiveHitResult(bestPrimitiveHit);
    }
    const sourceResult = hitTestPaneView(
      source._internal_paneViews(pane),
      x,
      y
    );
  ...
  if (
    bestPrimitiveHit === null || bestPrimitiveHit === void 0
      ? void 0
      : bestPrimitiveHit._internal_hit
  ) {
...
function buildPriceAxisViewsGetter(zOrder, priceScaleId) {
  return (source) => {
    ...
    const psId =
      (_b =
        (_a = source._internal_priceScale()) === null || _a === void 0
          ? void 0
          : _a._internal_id()) !== null && _b !== void 0
        ? _b
        : "";
    ...
    return (_d =
      (_c = source._internal_pricePaneViews) === null || _c === void 0
        ? void 0
        : _c.call(source, zOrder)) !== null && _d !== void 0
      ? _d
      : [];
  };
}
class PriceAxisWidget {
  constructor(pane, options, rendererOptionsProvider, side) {
    ...
    this._private__sourceBottomPaneViews = buildPriceAxisViewsGetter(
      "bottom",
      side
    );
    ...
    this._private__canvasBinding.subscribeSuggestedBitmapSizeChanged(
      this._private__canvasSuggestedBitmapSizeChangedHandler
    );
    ...
    this._private__topCanvasBinding.subscribeSuggestedBitmapSizeChanged(
      this._private__topCanvasSuggestedBitmapSizeChangedHandler
    );
    ...
    this._private__mouseEventHandler = new MouseEventHandler(
      this._private__topCanvasBinding.canvasElement,
      handler,
      {
  ...
  _internal_destroy() {
    ...
    this._private__topCanvasBinding.unsubscribeSuggestedBitmapSizeChanged(
      this._private__topCanvasSuggestedBitmapSizeChangedHandler
    );
    this._private__topCanvasBinding.dispose();
    this._private__canvasBinding.unsubscribeSuggestedBitmapSizeChanged(
      this._private__canvasSuggestedBitmapSizeChangedHandler
    );
  ...
  _internal_optimalWidth() {
    ...
    const ctx = ensureNotNull(
      this._private__canvasBinding.canvasElement.getContext("2d")
    );
    ctx.save();
    ...
    for (let j = views.length; j--; ) {
      const width = this._private__widthCache._internal_measureText(
        ctx,
    ...
    if (firstValue !== null && this._private__size !== null) {
      const topValue = this._private__priceScale._internal_coordinateToPrice(
        1,
        firstValue
      );
      const bottomValue = this._private__priceScale._internal_coordinateToPrice(
        this._private__size.height - 2,
        firstValue
      );
      tickMarkMaxWidth = Math.max(
        tickMarkMaxWidth,
    ...
    ctx.restore();
    const resultTickMarksMaxWidth =
      tickMarkMaxWidth || 34; /* Constants.DefaultOptimalWidth */
    const res = Math.ceil(
      rendererOptions._internal_borderSize +
        rendererOptions._internal_tickLength +
        rendererOptions._internal_paddingInner +
  ...
  _internal_setSize(newSize) {
    if (
      this._private__size === null ||
      !equalSizes(this._private__size, newSize)
    ) {
  ...
  _internal_paint(type) {
    ...
    if (type !== 1 /* InvalidationLevel.Cursor */) {
      ...
      const target = tryCreateCanvasRenderingTarget2D(
      ...
      if (target !== null) {
        ...
        this._private__pane._internal_drawAdditionalSources(
          target,
        ...
        this._private__pane._internal_drawAdditionalSources(
          target,
          this._private__sourcePaneViews
        );
        this._private__drawBackLabels(target);
      }
    }
    this._private__topCanvasBinding.applySuggestedBitmapSize();
    const topTarget = tryCreateCanvasRenderingTarget2D(
      this._private__topCanvasBinding
    );
    if (topTarget !== null) {
      ...
      this._private__pane._internal_drawAdditionalSources(
        topTarget,
        this._private__sourceTopPaneViews
      );
    }
  }
  ...
  _internal_update() {
    ...
    (_a = this._private__priceScale) === null || _a === void 0
      ? void 0
      : _a._internal_marks();
  }
  _private__mouseDownEvent(e) {
    if (
      this._private__priceScale === null ||
      this._private__priceScale._internal_isEmpty() ||
      !this._private__options.handleScale.axisPressedMouseMove.price
    ) {
      return;
    }
    const model = this._private__pane._internal_chart()._internal_model();
  ...
  _private__backLabels() {
    ...
    const addViewsForSources = (sources) => {
      for (let i = 0; i < sources.length; ++i) {
        ...
        const views = source._internal_priceAxisViews(
          this._private__pane._internal_state(),
          priceScale
        );
  ...
  _private__drawTickMarks(target) {
    ...
    if (priceScaleOptions.borderVisible && priceScaleOptions.ticksVisible) {
      target.useBitmapCoordinateSpace(
        ({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) => {
          ...
          const tickLength = Math.round(
            rendererOptions._internal_tickLength * horizontalPixelRatio
          );
          ctx.beginPath();
          for (const tickMark of tickMarks) {
            ctx.rect(
              Math.floor(tickMarkLeftX * horizontalPixelRatio),
              Math.round(tickMark._internal_coord * verticalPixelRatio) -
                tickOffset,
              tickLength,
              tickHeight
            );
    ...
    target.useMediaCoordinateSpace(({ context: ctx }) => {
      ...
      ctx.font = this._private__baseFont();
      ctx.fillStyle =
        (_a = priceScaleOptions.textColor) !== null && _a !== void 0
          ? _a
          : this._private__layoutOptions.textColor;
      ...
      const yMidCorrections = tickMarks.map((mark) =>
        this._private__widthCache._internal_yMidCorrection(
          ctx,
          mark._internal_label
        )
      );
      for (let i = tickMarks.length; i--; ) {
        ...
        ctx.fillText(
          ...
          textLeftX,
          tickMark._internal_coord + yMidCorrections[i]
        );
      }
    });
  }
  _private__alignLabels() {
    ...
    const views = [];
    const orderedSources = this._private__priceScale
      ._internal_orderedSources()
      .slice(); // Copy of array
    ...
    const isDefault =
      this._private__priceScale ===
      paneState._internal_defaultVisiblePriceScale();
    if (isDefault) {
      this._private__pane
        ._internal_state()
        ._internal_orderedSources()
    ...
    const updateForSources = (sources) => {
      sources.forEach((source) => {
        const sourceViews = source._internal_priceAxisViews(
          paneState,
          priceScale
        );
    ...
    views.forEach((view) =>
      view._internal_setFixedCoordinate(view._internal_coordinate())
    );
  ...
  _private__fixLabelOverlap(views, rendererOptions, center) {
    ...
    for (const view of views) {
      ...
      if (
        coordinate > this._private__size.height - halfHeight &&
        coordinate < this._private__size.height + halfHeight
      ) {
        view._internal_setFixedCoordinate(
          this._private__size.height - halfHeight
        );
      }
    }
  ...
  _private__drawBackLabels(target) {
    ...
    views.forEach((view) => {
      if (view._internal_isAxisLabelVisible()) {
        const renderer = view._internal_renderer(
          ensureNotNull(this._private__priceScale)
        );
  ...
  _private__drawCrosshairLabel(target) {
    ...
    views.forEach((arr) => {
      arr.forEach((view) => {
        view
          ._internal_renderer(ensureNotNull(this._private__priceScale))
          ._internal_draw(target, ro, this._private__widthCache, align);
      });
    });
  }
  _private__setCursor(type) {
    this._private__cell.style.cursor =
      type === 1 /* CursorType.NsResize */ ? "ns-resize" : "default";
  }
  ...
  _private__baseFont() {
    return makeFont(
      this._private__layoutOptions.fontSize,
      this._private__layoutOptions.fontFamily
    );
  }
}
...
function sourceBottomPaneViews$1(source, pane) {
  var _a, _b;
  return (_b =
    (_a = source._internal_bottomPaneViews) === null || _a === void 0
      ? void 0
      : _a.call(source, pane)) !== null && _b !== void 0
    ? _b
    : [];
}
function sourcePaneViews$1(source, pane) {
  var _a, _b;
  return (_b =
    (_a = source._internal_paneViews) === null || _a === void 0
      ? void 0
      : _a.call(source, pane)) !== null && _b !== void 0
    ? _b
    : [];
...
function sourceLabelPaneViews(source, pane) {
  var _a, _b;
  return (_b =
    (_a = source._internal_labelPaneViews) === null || _a === void 0
      ? void 0
      : _a.call(source, pane)) !== null && _b !== void 0
    ? _b
    : [];
...
function sourceTopPaneViews$1(source, pane) {
  var _a, _b;
  return (_b =
    (_a = source._internal_topPaneViews) === null || _a === void 0
      ? void 0
      : _a.call(source, pane)) !== null && _b !== void 0
    ? _b
    : [];
...
class PaneWidget {
  ...
  _internal_mouseMoveEvent(event) {
    ...
    this._private__chart._internal_setCursorStyle(
      (_a =
        hitTest === null || hitTest === void 0
          ? void 0
          : hitTest._internal_cursorStyle) !== null && _a !== void 0
        ? _a
        : null
    );
  ...
  _internal_touchEndEvent(event) {
    if (
      this._internal_chart()._internal_options().trackingMode.exitMode ===
  ...
  _internal_setPriceAxisSize(width, position) {
    ...
    ensureNotNull(priceAxisWidget)._internal_setSize(
      size$1({ width, height: this._private__size.height })
    );
  }
  _internal_getSize() {
  ...
  _private__fireClickedDelegate(event) {
    ...
    if (this._private__clicked._internal_hasListeners()) {
      this._private__clicked._internal_fire(
        this._private__model()
          ._internal_timeScale()
          ._internal_coordinateToIndex(x),
        { x, y },
  ...
  _private__recreatePriceAxisWidgets() {
    ...
    const leftAxisVisible = this._private__state
      ._internal_leftPriceScale()
      ._internal_options().visible;
    const rightAxisVisible = this._private__state
      ._internal_rightPriceScale()
      ._internal_options().visible;
    ...
    const rendererOptionsProvider = chart
      ._internal_model()
      ._internal_rendererOptionsProvider();
    if (leftAxisVisible && this._private__leftPriceAxisWidget === null) {
      this._private__leftPriceAxisWidget = new PriceAxisWidget(
        this,
        chart._internal_options(),
  ...
  _private__preventScroll(event) {
    return (
      (event._internal_isTouch && this._private__longTap) ||
      this._private__startTrackPoint !== null
    );
  }
  ...
  _private__endScroll(event) {
    ...
    if (this._private__scrollXAnimation !== null) {
      ...
      this._private__scrollXAnimation._internal_start(
        timeScale._internal_rightOffset(),
        startAnimationTime
      );
      if (
  ...
  _private__mouseTouchDownEvent() {
    ...
    if (
      priceScale._internal_isEmpty() ||
      this._private__model()._internal_timeScale()._internal_isEmpty()
    ) {
      return;
    }
  }
  ...
  _private__pressedMouseTouchMoveEvent(event) {
    ...
    if (
      (!scrollOptions.pressedMouseMove || event._internal_isTouch) &&
    ...
    if (
      this._private__startScrollingPos === null &&
      !this._private__preventScroll(event)
    ) {
    ...
    if (
      ...
      if (
        ...
        this._private__scrollXAnimation = new KineticAnimation(
          0.2 /* KineticScrollConstants.MinScrollSpeed */ / barSpacing,
          7 /* KineticScrollConstants.MaxScrollSpeed */ / barSpacing,
          0.997 /* KineticScrollConstants.DumpingCoeff */,
          15 /* KineticScrollConstants.ScrollMinMove */ / barSpacing
        );
...
class PriceAxisStub {
  constructor(side, options, params, borderVisible, bottomColor) {
    ...
    this._private__size = size$1({ width: 0, height: 0 });
    this._private__canvasSuggestedBitmapSizeChangedHandler = () =>
      this._internal_paint(3 /* InvalidationLevel.Full */);
    this._private__isLeft = side === "left";
    this._private__rendererOptionsProvider =
  ...
  _private__drawBorder({
    ...
    const horzBorderSize = Math.floor(
      this._private__rendererOptionsProvider._internal_options()
        ._internal_borderSize * horizontalPixelRatio
    );
    const vertBorderSize = Math.floor(
      this._private__rendererOptionsProvider._internal_options()
        ._internal_borderSize * verticalPixelRatio
    );
...
function buildTimeAxisViewsGetter(zOrder) {
  return (source) => {
    var _a, _b;
    return (_b =
      (_a = source._internal_timePaneViews) === null || _a === void 0
        ? void 0
        : _a.call(source, zOrder)) !== null && _b !== void 0
      ? _b
      : [];
  };
}
...
class TimeAxisWidget {
  ...
  _internal_mouseDownEvent(event) {
    ...
    if (
      ...
      !this._private__chart._internal_options().handleScale.axisPressedMouseMove
        .time
    ) {
      return;
    }
  ...
  _internal_mouseDoubleClickEvent() {
    if (
      this._private__chart._internal_options().handleScale.axisDoubleClickReset
        .time
    ) {
  ...
  _internal_mouseEnterEvent() {
    if (
      this._private__chart._internal_model()._internal_options().handleScale
        .axisPressedMouseMove.time
    ) {
  ...
  _internal_setSizes(timeAxisSize, leftStubWidth, rightStubWidth) {
    ...
    if (this._private__leftStub !== null) {
      this._private__leftStub._internal_setSize(
        size$1({ width: leftStubWidth, height: timeAxisSize.height })
      );
    }
    if (this._private__rightStub !== null) {
      this._private__rightStub._internal_setSize(
        size$1({ width: rightStubWidth, height: timeAxisSize.height })
      );
    }
  }
  ...
  _private__drawAdditionalSources(target, axisViewsGetter) {
    ...
    for (const source of sources) {
      drawSourcePaneViews(
        axisViewsGetter,
        (renderer) => drawBackground(renderer, target, false, undefined),
        source,
        undefined
      );
    }
    for (const source of sources) {
      drawSourcePaneViews(
        axisViewsGetter,
        (renderer) => drawForeground(renderer, target, false, undefined),
        source,
        undefined
      );
    }
  ...
  _private__drawBackground({ context: ctx, bitmapSize }) {
    clearRect(
      ...
      bitmapSize.width,
      bitmapSize.height,
      this._private__chart._internal_model()._internal_backgroundBottomColor()
    );
  }
  ...
  _private__drawTickMarks(target) {
    const timeScale = this._private__chart
      ._internal_model()
      ._internal_timeScale();
    ...
    let maxWeight = tickMarks.reduce(
    ...
    if (
      ...
      maxWeight < 50 /* TickMarkWeight.Day */
    ) {
      maxWeight = 30 /* TickMarkWeight.Hour1 */;
    }
    const rendererOptions = this._private__getRendererOptions();
      ...
      target.useBitmapCoordinateSpace(
        ({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) => {
          ...
          const tickLen = Math.round(
            rendererOptions._internal_tickLength * verticalPixelRatio
          );
          for (let index = tickMarks.length; index--; ) {
            const x = Math.round(
              tickMarks[index]._internal_coord * horizontalPixelRatio
            );
    ...
    target.useMediaCoordinateSpace(({ context: ctx }) => {
      ...
      for (const tickMark of tickMarks) {
        if (tickMark._internal_weight < maxWeight) {
          const coordinate = tickMark._internal_needAlignCoordinate
            ? this._private__alignTickMarkLabelCoordinate(
                ctx,
  ...
  _private__alignTickMarkLabelCoordinate(ctx, coordinate, labelText) {
    const labelWidth = this._private__widthCache._internal_measureText(
      ctx,
      labelText
    );
    const labelWidthHalf = labelWidth / 2;
    ...
    if (leftTextCoordinate < 0) {
      ...
      coordinate =
        coordinate -
        Math.abs(this._private__size.width - (leftTextCoordinate + labelWidth));
    }
    return coordinate;
  }
  ...
  _private__setCursor(type) {
    this._private__cell.style.cursor =
      type === 1 /* CursorType.EwResize */ ? "ew-resize" : "default";
  }
  _private__recreateStubs() {
    ...
    const rendererOptionsProvider = this._private__chart
      ._internal_model()
      ._internal_rendererOptionsProvider();
    const params = {
...
class ChartWidget {
  constructor(container, options) {
    ...
    this._private__model
      ...
      ._internal_optionsApplied()
      ._internal_subscribe(
        this._private__model._internal_fullUpdate.bind(this._private__model),
        this
      );
    this._private__model
      ._internal_priceScalesOptionsChanged()
  ...
  _internal_resize(width, height, forceRepaint = false) {
    ...
    if (forceRepaint) {
      this._private__drawImpl(
        InvalidateMask._internal_full(),
        performance.now()
      );
    } else {
  ...
  _internal_applyOptions(options) {
    const currentlyHasMouseWheelListener = shouldSubscribeMouseWheel(
      this._private__options
    );
    ...
    const shouldHaveMouseWheelListener = shouldSubscribeMouseWheel(
      this._private__options
    );
  ...
  _private__applyAutoSizeOptions(options) {
    ...
    if (
      ...
      this._internal_resize(
        options.width || this._private__width,
        options.height || this._private__height
      );
    }
  }
  /**
  ...
  _private__traverseLayout(ctx) {
    ...
    if (this._private__isLeftAxisVisible()) {
      ...
      const leftAxisBitmapWidth = ensureNotNull(
        firstPane._internal_leftPriceAxisWidget()
      )._internal_getBitmapSize().width;
    ...
    if (this._private__isRightAxisVisible()) {
      ...
      const rightAxisBitmapWidth = ensureNotNull(
        firstPane._internal_rightPriceAxisWidget()
      )._internal_getBitmapSize().width;
    ...
    const drawStub = (position, targetX, targetY) => {
      const stub = ensureNotNull(
        position === "left"
          ? this._private__timeAxisWidget._internal_leftStub()
          : this._private__timeAxisWidget._internal_rightStub()
      );
      stub._internal_drawBitmap(ensureNotNull(ctx), targetX, targetY);
    };
    // draw time scale and stubs
    if (this._private__options.timeScale.visible) {
      const timeAxisBitmapSize =
        this._private__timeAxisWidget._internal_getBitmapSize();
      if (ctx !== null) {
        let targetX = 0;
        if (this._private__isLeftAxisVisible()) {
        ...
        this._private__timeAxisWidget._internal_drawBitmap(
          ctx,
          targetX,
          totalHeight
        );
  ...
  _private__adjustSizeImpl() {
    ...
    for (
      ...
      paneWidget._internal_setSize(
        size$1({ width: paneWidth, height: paneHeight })
      );
      if (this._private__isLeftAxisVisible()) {
  ...
  _private__onMousewheel(event) {
    if (
      (event.deltaX === 0 || !this._private__options.handleScroll.mouseWheel) &&
      (event.deltaY === 0 || !this._private__options.handleScale.mouseWheel)
    ) {
      return;
    }
    const scrollSpeedAdjustment =
      this._private__determineWheelSpeedAdjustment(event);
    ...
    if (deltaY !== 0 && this._private__options.handleScale.mouseWheel) {
      ...
      const scrollPosition =
        event.clientX - this._private__element.getBoundingClientRect().left;
  ...
  _private__drawImpl(invalidateMask, time) {
    ...
    if (
      invalidationType === 3 /* InvalidationLevel.Full */ ||
      ...
      if (
        ((_a = this._private__invalidateMask) === null || _a === void 0
          ? void 0
          : _a._internal_fullInvalidation()) === 3 /* InvalidationLevel.Full */
      ) {
        ...
        this._private__applyTimeScaleInvalidations(
          this._private__invalidateMask,
          time
        );
    ...
    switch (invalidation._internal_type) {
      ...
      case 5 /* TimeScaleInvalidationType.Animation */:
        if (!invalidation._internal_value._internal_finished(time)) {
          timeScale._internal_setRightOffset(
            invalidation._internal_value._internal_getPosition(time)
          );
        }
        break;
    }
      ...
      this._private__drawRafId = window.requestAnimationFrame((time) => {
        ...
        if (this._private__invalidateMask !== null) {
          ...
          for (const tsInvalidation of mask._internal_timeScaleInvalidations()) {
            if (
              tsInvalidation._internal_type ===
                5 /* TimeScaleInvalidationType.Animation */ &&
              !tsInvalidation._internal_value._internal_finished(time)
            ) {
              this._internal_model()._internal_setTimeScaleAnimation(
                tsInvalidation._internal_value
              );
              break;
            }
          }
  ...
  _private__getMouseEventParamsImpl(index, point, event) {
    ...
    if (index !== null) {
      const timePoint =
        (_a = this._private__model
          ._internal_timeScale()
          ._internal_indexToTimeScalePoint(index)) === null || _a === void 0
          ? void 0
          : _a._internal_originalTime;
    ...
    const hoveredSeries =
      hoveredSource !== null && hoveredSource._internal_source instanceof Series
        ? hoveredSource._internal_source
        : undefined;
    const hoveredObject =
      hoveredSource !== null && hoveredSource._internal_object !== undefined
    ...
    return {
      ...
      _internal_touchMouseEventData:
        event !== null && event !== void 0 ? event : undefined,
    };
  }
  _private__onPaneWidgetClicked(time, point, event) {
    this._private__clicked._internal_fire(() =>
      this._private__getMouseEventParamsImpl(time, point, event)
    );
  }
    ...
    if (!("ResizeObserver" in window)) {
      ...
      this._private__observer = new ResizeObserver((entries) => {
        const containerEntry = entries.find(
          (entry) => entry.target === this._private__container
        );
        if (!containerEntry) {
          return;
        }
...
function shouldSubscribeMouseWheel(options) {
  return Boolean(
    options.handleScroll.mouseWheel || options.handleScale.mouseWheel
  );
}
...
function getCustomSeriesPlotRow(
  time,
  index,
  item,
  originalTime,
...
function weightByTime(currentDate, prevDate) {
  ...
  for (let i = intradayWeightDivisors.length - 1; i >= 0; --i) {
    if (
      ...
      Math.floor(
        currentDate.getTime() / intradayWeightDivisors[i]._internal_divisor
      )
    ) {
...
function seriesRowsFirsAndLastTime(seriesRows) {
  ...
  return {
    ...
    _internal_lastTime:
      seriesRows[seriesRows.length - 1]._internal_time._internal_timestamp,
  };
}
function seriesUpdateInfo(seriesRows, prevSeriesRows) {
  ...
  if (firstAndLastTime !== undefined && prevFirstAndLastTime !== undefined) {
    return {
      _internal_lastBarUpdatedOrNewBarsAddedToTheRight:
        firstAndLastTime._internal_lastTime >=
          prevFirstAndLastTime._internal_lastTime &&
        firstAndLastTime._internal_firstTime >=
          prevFirstAndLastTime._internal_firstTime,
    };
  }
  return undefined;
}
...
class DataLayer {
  ...
  _internal_setSeriesData(series, data) {
    ...
    if (data.length !== 0) {
      ...
      const createPlotRow = getSeriesPlotRowCreator(
        series._internal_seriesType()
      );
      const dataToPlotRow = series._internal_customSeriesPlotValuesBuilder();
      const customWhitespaceChecker =
        series._internal_customSeriesWhitespaceCheck();
      seriesRows = data.map((item, index) => {
        const time = timeConverter(item.time);
        let timePointData = this._private__pointDataByTimePoint.get(
          time._internal_timestamp
        );
    ...
    if (isTimeScaleAffected) {
      ...
      this._private__pointDataByTimePoint.forEach((pointData) => {
        newTimeScalePoints.push({
          ...
          pointData,
          _internal_originalTime: timeScalePointTime(
            pointData._internal_mapping
          ),
        });
      });
      ...
      firstChangedPointIndex =
        this._private__replaceTimeScalePoints(newTimeScalePoints);
    }
    return this._private__getUpdateResponse(
      series,
      firstChangedPointIndex,
      seriesUpdateInfo(
        this._private__seriesRowsBySeries.get(series),
        prevSeriesRows
      )
    );
  }
  ...
  _internal_updateSeriesData(series, data) {
    ...
    if (
      ...
      throw new Error(
        `Cannot update oldest data, last time=${lastSeriesTime._internal_timestamp}, new time=${time._internal_timestamp}`
      );
    }
    let pointDataAtTime = this._private__pointDataByTimePoint.get(
      time._internal_timestamp
    );
    ...
    const plotRow = createPlotRow(
      time,
      pointDataAtTime._internal_index,
      data,
      originalTime,
      dataToPlotRow,
      customWhitespaceChecker
    ...
    const newPoint = {
      ...
      _internal_originalTime: timeScalePointTime(
        pointDataAtTime._internal_mapping
      ),
    };
...
class PriceScaleApi {
  ...
  applyOptions(options) {
    this._private__chartWidget
      ._internal_model()
      ._internal_applyPriceScaleOptions(this._private__priceScaleId, options);
  }
  options() {
...
class SeriesApi {
  ...
  barsInLogicalRange(range) {
    ...
    if (
      ...
      dataFirstBarInRange._internal_index > dataLastBarInRange._internal_index
    ) {
      return {
  ...
  dataByIndex(logicalIndex, mismatchDirection) {
    const data = this._internal__series
      ._internal_bars()
      ._internal_search(logicalIndex, mismatchDirection);
    if (data === null) {
  ...
  setMarkers(data) {
    ...
    const convertedMarkers = data.map((marker) =>
      Object.assign(Object.assign({}, marker), {
  ...
  markers() {
    return this._internal__series._internal_markers().map((internalItem) => {
      const { originalTime, time } = internalItem,
...
class TimeScaleApi {
  ...
  scrollToPosition(position, animated) {
    ...
    this._private__timeScale._internal_scrollToOffsetAnimated(
      position,
      1000 /* Constants.AnimationDurationMs */
    );
  }
  scrollToRealTime() {
  ...
  getVisibleRange() {
    ...
    return {
      from:
        (_a = timeRange.from._internal_businessDay) !== null && _a !== void 0
          ? _a
          : timeRange.from._internal_timestamp,
      to:
        (_b = timeRange.to._internal_businessDay) !== null && _b !== void 0
          ? _b
  ...
  timeToCoordinate(time) {
    ...
    const timePointIndex = this._private__timeScale._internal_timeToIndex(
      timePoint,
      false
    );
  ...
  options() {
    return Object.assign(
      Object.assign({}, this._private__timeScale._internal_options()),
...
class ChartApi {
  constructor(container, options) {
    ...
    this._private__chartWidget
      ._internal_clicked()
      ._internal_subscribe((paramSupplier) => {
        if (this._private__clickedDelegate._internal_hasListeners()) {
          this._private__clickedDelegate._internal_fire(
            this._private__convertMouseParams(paramSupplier())
          );
        }
      }, this);
    this._private__chartWidget
      ...
      ._internal_subscribe((paramSupplier) => {
        if (this._private__crosshairMovedDelegate._internal_hasListeners()) {
          this._private__crosshairMovedDelegate._internal_fire(
            this._private__convertMouseParams(paramSupplier())
          );
        }
      }, this);
  ...
  addCustomSeries(customPaneView, options) {
    ...
    const defaults = Object.assign(
      Object.assign({}, customStyleDefaults),
  ...
  addBaselineSeries(options) {
    return this._private__addSeriesImpl(
      "Baseline",
  ...
  _private__addSeriesImpl(type, styleDefaults, options = {}, customPaneView) {
    ...
    const series = this._private__chartWidget
      ._internal_model()
      ._internal_createSeries(type, strictOptions, customPaneView);
...
const customSeriesDefaultOptions = Object.assign(
  Object.assign({}, seriesOptionsDefaults),
  customStyleDefaults
);
/**
