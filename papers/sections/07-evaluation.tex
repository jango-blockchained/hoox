...
\begin{itemize}
  \item \texttt{hoox perf fastpath run --n 200 --concurrency 8} followed by \texttt{hoox perf fastpath report --from 1h} (requires \texttt{@hoox-sh/hoox-cli v0.9.3+}) surfaces the detailed hop table.
  \item \texttt{hoox trace} (requires \texttt{@hoox-sh/hoox-cli v0.9.3+}) pulls raw events, metrics, and sampled traces by probe\_id for full per-request flame-graph style reconstruction.
