    ...
    if (!target) {
      return new Response(
        JSON.stringify({
          success: false,
          worker: "webhook-receiver",
          error: "Missing target worker specification",
        }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    if (!targetUrl) {
      return new Response(
        JSON.stringify({
          success: false,
          worker: "webhook-receiver",
          error: `Invalid target worker specified: ${target}`,
        }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
...
async function forwardToWorker(targetUrl, standardizedPayload) {
  ...
  try {
    console.log(
      `Forwarding request ID ${standardizedPayload.requestId} to: ${fullUrl}`
    );
    ...
    console.log(
      `Response from ${fullUrl} (Status ${response.status}):`,
      responseData
    );
