...
JSON bodies may include `"test": true` to request exchange testnet execution (Binance/Bybit). The flag is forwarded on the `TRADE_SERVICE` webhook call. **Plaintext extraction does not set `test`** (always live) — use structured JSON email bodies for sandbox trades. See [Test Trading](/enduser/guides/test-trading).
...
- `EmailSignalSchema` validates the signal structure (exchange, action as enum, symbol, quantity with default 100, optional price/leverage/`test`)
