...
2. **Coverage Thresholds**: The monorepo enforces a **minimum 80% coverage threshold** across all core execution paths (`packages/cli`, `packages/shared`, `workers/hoox-worker`, `workers/trade-worker`).
...
- **[Debugging Telemetry Runbook](debugging)** — Learn how to trace active V8 memory, tail logs, and audit SQL execution.
- **[Local Development Setup](local-dev)** — Configure Wrangler and Docker compose to run testbeds.
