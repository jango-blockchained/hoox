...
**File:** `workers/hoox-worker/src/index.ts`
...
| ---------------------- | -------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------ |
| **hoox**               | API key in body (timing-safe) + IP allowlist + kill switch | `POST /`, `POST /webhook`                                  | `GET /health`                                    |
| **hoox operator plane** | Bearer `OPERATOR_API_KEY` (`requireOperatorAuth`) | `GET /v1/health`, `/v1/workers`, SSE `/v1/trades|logs/stream` | Prefer Access on mgmt hostname               |
...
| **telegram-worker**    | `X-Internal-Auth-Key`                        | `POST /alert`, `POST /process`                                   | `GET /health`, `POST /webhook` (Telegram secret + chat allowlist) |
