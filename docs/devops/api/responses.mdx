...
To enforce this response contract without writing redundant JSON wrappers in every worker, we use the standardized `Errors` factory from the shared monorepo package `@hoox-sh/hoox-shared/errors`:
