...
- **JSON Payload**:
  ...
  ```
- **`test` (optional boolean)**: when `true`, routes to exchange testnet (Binance/Bybit). MEXC returns `TEST_TRADING_UNSUPPORTED`. Idempotency keys are mode-split (`:live` vs `:test`). Prefer `BINANCE_TESTNET_*` / `BYBIT_TESTNET_*` secrets. See [Test Trading](/enduser/guides/test-trading).
...
- **[Request Payload Schemas](payloads)** — Analyze the complete JSON request schemas and type rules.
- **[Standard Response Schemas](responses)** — Check error factories and normal JSON envelopes.
