...
<Warning>
  ...
  sync`. Never commit secrets to version control.
</Warning>
...
git clone --recursive https://github.com/hoox-sh/hoox-setup.git my-hoox-app
cd my-hoox-app
...
```jsonc
// Already defined in workers/hoox-worker/wrangler.jsonc
"durable_objects": {
  "bindings": [
    {
...
- [ ] Cron triggers scheduled (agent-worker: 1–1440 min via `triggers.crons`, default 15 min; email-worker: every 5 min)
