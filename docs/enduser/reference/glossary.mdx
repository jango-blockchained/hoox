...
| **Test trading**       | Sandbox execution mode enabled by `"test": true` on a webhook/queue payload. Routes to exchange testnet hosts (Binance/Bybit); ledger status `TEST_EXECUTED`.        |
| **TEST_EXECUTED**      | D1 trade status written for testnet fills so live reports and stats can exclude them.                                                                                 |
| **Testnet position id**| Position primary key pattern `{exchange}-testnet-{symbol}-{side}` isolating sandbox exposure from live OPEN rows.                                                     |
