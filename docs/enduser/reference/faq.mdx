...
```bash
git clone --recursive https://github.com/hoox-sh/hoox-setup.git
```
...
Test fills are stored as `TEST_EXECUTED` with `*-testnet-*` position ids so they do not mix with live exposure. See **[Test Trading](/enduser/guides/test-trading)**.
...
To change the interval, update `triggers.crons` and redeploy. See [agent-worker Cron setup](/devops/workers/agent-worker#cron-schedule-11440-minutes).
...
<Note>
  Open an issue on
  [GitHub](https://github.com/hoox-sh/hoox-setup/issues) or join our
