...
- **Hoox Implementation**: Drives the Gateway's **Idempotency Engine** inside `workers/hoox-worker`.
- **Why it Matters**:
...
<Tip>
  Every single one of these services is supported by the Hoox CLI! You can check
  database tables using `hoox db query`, provision R2 buckets with `hoox infra
  r2 create`, and sync your KV manifest using `hoox config kv apply-manifest`
  instantly.
</Tip>
