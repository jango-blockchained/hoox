...
description: "How the agent-worker runs configurable cron checks (1–1440 minutes) to calculate trailing stops, evaluate max daily drawdowns, and govern a multi-provider fallback AI gateway."
---
...
```mermaid
graph TD
    Cron[Cron Trigger 1–1440 min] --> Query[Query D1 & Exchange Balances]
...
<Tip>
  AI gateway defaults and trailing deviations update in real-time via KV — no
  redeploy needed for risk profile tweaks. The **cron interval** is set in
  `wrangler.jsonc` → `triggers.crons` (1–1440 minutes); change it and redeploy
  the agent-worker. See the [agent-worker profile](/devops/workers/agent-worker).
</Tip>

### 🔗 Next Steps

- **[Zero Trust & Secret Security](../guides/secrets-security)** — Lock down your AI endpoints and exchange API keys.
- **[TUI Operations Cockpit](../guides/tui)** — View live position drawdowns and trailing stops visually in your terminal.
