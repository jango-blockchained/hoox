...
Full operator guide: **[Test Trading](/enduser/guides/test-trading)**.
...
| :---------------- | :----------------------: | :-------------------------------------- |
| **`LONG`**        |        **`BUY`**         | Opens a long position                   |
| **`SHORT`**       |        **`SELL`**        | Opens a short position                  |
| **`CLOSE_LONG`**  |        **`SELL`**        | Closes an existing long (`reduceOnly`)  |
| **`CLOSE_SHORT`** |        **`BUY`**         | Closes an existing short (`reduceOnly`) |
...
<Tip>
  By utilizing time-series logging via Cloudflare Analytics Engine, Hoox tracks
  the exact execution duration of each pipeline step. You can audit the latency
  breakdown (e.g., Gateway auth took `1.2ms`, Trade-worker signing took `0.8ms`,
  Bybit API roundtrip took `18.5ms`) directly in your Next.js dashboard!
</Tip>

### 🔗 Next Steps

- **[Autonomous AI Risk Management](ai-risk-manager)** — Learn how agent-worker tracks D1 entries to manage trailing stops and drawdowns.
- **[CLI Reference Manual](../reference/cli-commands)** — Review commands to manage D1 tables, perform dry runs, and tail logs.
- **[Error Models & Status Codes](../reference/api-endpoints#d-standard-platform-error-models)** — Understand all gateway error responses.
