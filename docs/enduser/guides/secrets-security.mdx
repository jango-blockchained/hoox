...
# 2. List edge secret names for a worker (values are never printed)
hoox secrets list trade-worker

# 3. After key rotation: push mesh/system secrets only
hoox secrets sync --system
# Alias: hoox secrets sync --required

# 4. Push all declared secrets from .dev.vars
