...
<Warning>
  Running `hoox db apply` compiles migrations and seeds the database locally.
  For production deployment, you **must** append the `--remote` flag to execute
  operations directly on your active Cloudflare D1 instance.
</Warning>
...
<ol>
  <li>
    <strong>Always write reversible migrations</strong> — include both{" "}
    <code>UP</code> and <code>DOWN</code> scripts.
  </li>
  <li>
    <strong>Never modify existing migrations</strong> — create new ones for
    schema changes.
  </li>
  <li>
    <strong>Test locally first</strong> — run <code>hoox db apply</code> without{" "}
    <code>--remote</code> to verify.
  </li>
  <li>
    <strong>Backup before migrating</strong> — export your production database
    first.
  </li>
</ol>
...
<Warning>
  Wiping your production D1 database is an irreversible operation. It will
  permanently delete all trade logs, position records, and asset histories.
  **Always** execute `hoox db export --remote` before running a reset!
</Warning>
