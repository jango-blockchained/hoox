...
description: "Deploy your first edge-native trading infrastructure on the Cloudflare Edge and fire a simulated trade webhook in under 5 minutes."
---
...
See **[Test Trading](/enduser/guides/test-trading)** for credential setup, D1 isolation, and dashboard filters.
...
| `action`   | `string`  | **Yes**  | `LONG`, `SHORT`, `CLOSE_LONG`, or `CLOSE_SHORT`.                                                 |
...
| `test`     | `boolean` |    No    | When `true`, use exchange testnet (Binance/Bybit). Rejected for MEXC. Prefer `*_TESTNET_*` secrets. Default: live. |
